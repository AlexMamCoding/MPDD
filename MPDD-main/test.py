...
if __name__ == '__main__':
    ...
    args.test_json = os.path.join(args.data_rootpath, 'labels', 'Testing_files.json')
    ...
    model = ourModel(opt)
    # 修正：添加map_location参数，确保模型可以在CPU或GPU上加载
    model.load_state_dict(torch.load(args.train_model, map_location=args.device))
